...
Salient::Salient()
{
	...
	//nn->loadParams("./model/param_hog.model");
...
cv::Mat Salient::salientDetectSR(const cv::Mat& im) {
	cv::Mat salMap;
	salientSR.computeSaliency(im, salMap);
	return salMap;
}
cv::Mat Salient::salientDetectFG(const cv::Mat& im) {
	cv::Mat salMap;
	salientFG.computeSaliency(im, salMap);
	return salMap;
}
cv::Mat Salient::salientDetectMotion(const cv::Mat& im) {
	cv::Mat salMap;
	salientMotion.computeSaliency(im, salMap);
	return salMap;
}

cv::Mat Salient::salientDetectFT(const cv::Mat& im) {
...
std::vector<cv::Rect> Salient::findBoundingBoxes(const cv::Mat& im){
	...
	for (; pContour; pContour = pContour->h_next) {
		...
		if (bbox.width > 2*bbox.height || bbox.height > 2*bbox.width) {
...
cv::Mat Salient::adaptBinarize(const cv::Mat& im) {
	...
	cv::Mat borderImg;
	cv::copyMakeBorder(im, borderImg, halfSize, halfSize, halfSize, halfSize, cv::BORDER_CONSTANT, 0);
